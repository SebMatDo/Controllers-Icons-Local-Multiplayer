...
class ControllerIcons_Item:
	func _init(tree: Tree, root: TreeItem, path: String, is_default: bool, player : int):
		...
		controller_icon_key.force_type = 0
		controller_icon_key.player = player
		controller_icon_key.path = path
		
		controller_icon_joy = ControllerIconTexture.new()
		controller_icon_joy.player = player
		controller_icon_joy.force_type = 1
...
func populate(editor_interface: EditorInterface) -> void:
	...
	for data in ControllerIcons._custom_input_actions:
		var child := ControllerIcons_Item.new(n_tree, root, data, data in default_actions, player)
