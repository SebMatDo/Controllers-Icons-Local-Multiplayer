...
# -1 is not a device, you should update the device of this, up to 8 controllers
var player_controllers : Array[int] = [-1,-1,-1,-1,-1,-1,-1,-1]

var _default_player_controllers : Array[int] = [0,1,2,3,4,5,6,7]
...
func update_player_device(_player : PlayerDevices, _device : int) -> void:
	player_controllers[_player] = _device
...
# Return joypad type using an fallback, settings device is used
func get_joypad_type(controller) -> ControllerSettings.Devices:
	return Mapper._get_joypad_type(controller, _settings.joypad_fallback)

# This parse the path to an texture using icons
func parse_path(path: String, input_type : InputType = InputType.KEYBOARD_MOUSE, last_controller = 0) -> Texture:
...
# Text to speak
func parse_path_to_tts(path: String, input_type: int = -1, controller: int = -1) -> String:
	if input_type == -1:
		return ""
...
func get_matching_event(path: String, input_type: InputType = InputType.KEYBOARD_MOUSE , controller: int = -1 ) -> InputEvent:
	var events : Array
